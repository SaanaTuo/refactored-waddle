...
use functions
implicit none
integer::ios,i,n0,j,n1,i2, timesteps !n0 is the orig amount of particles, n1 filtered amount
...
character(len=80)::file1,arg1,arg2,arg3,file2,file_particle,filepath
...
loop1:do i=1,n0 !every particle one by one
   write(file_particle,'(a21,i1,a4)'),"trajectory_for_python",i,".xyz" !Additional. Supposed to be subroutine but couldn't think of a good way to make one.  
   !Open the file
   open(unit=13,file=filepath//file_particle,iostat=ios,status='new')!Note again that if you run the program more than once file of this name already exists. Delete it. Or change file_particle.
 if (ios/=0) then  
   ...
   loop2:do i2=1,500
      ...
      write(13,'(a1,3e20.10)'),"A",particles(i)%position
   end do loop2
...
file2='output13.dat'  !name of the output file
